...
class BBBIMU:
    def __init__(self, bus):
        ...
        try:
            ...
            print("[IMU] BNO055 IMU initialized in 9-DOF NDOF mode (Magnetometer Enabled).")
...
def main():
    ...
    try:
        ...
        while True:
            ...
            # Print state with live calibration levels
            wd_state = "DRIVING" if (loop_start - last_key_time < 0.3) else "TIMEOUT"
            cal_status = imu.sensor.calibration_status if imu.initialized else (0, 0, 0, 0)
            sys_c, gyro_c, accel_c, mag_c = cal_status
            sys.stdout.write(
                f"\r[CAL -> S:{sys_c} G:{gyro_c} A:{accel_c} M:{mag_c}] State: [{wd_state:^7}] | Cmd: V={V:+.2f} W={W:+.2f} | Pose: x={x:>5.2f} y={y:>5.2f} yaw={math.degrees(psi):>+6.1f}°"
            )
            sys.stdout.flush()
        ...
        # Read and save calibration offsets
        if imu.initialized and imu.sensor is not None:
            print("\nReading calibration offsets from sensor registers...")
            try:
                import json
                offsets = {
                    "accel": list(imu.sensor.offsets_accelerometer),
                    "gyro": list(imu.sensor.offsets_gyroscope),
                    "mag": list(imu.sensor.offsets_magnetometer),
                    "accel_radius": imu.sensor.radius_accelerometer,
                    "mag_radius": imu.sensor.radius_magnetometer
                }
                cal_file = "/home/debian/Robot/bno055_calibration.json"
                with open(cal_file, "w") as f:
                    json.dump(offsets, f, indent=4)
                print(f"✓ 9-axis calibration successfully saved to {cal_file}")
                print(offsets)
            except Exception as e:

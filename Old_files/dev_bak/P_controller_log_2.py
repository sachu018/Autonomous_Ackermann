#!/usr/bin/env python3
"""
BBB_Waypoint_Follower_Merged.py
-------------------------------
BeagleBone Black version of closed-loop CSV waypoint trajectory tracking.
Combines robust hardware slew-rate limiting from straightline tests with 
waypoint P-controller logic and comprehensive IMU/Encoder logging.
"""
...
dt = 0.1              # Control loop rate (10 Hz for large UGV stability)

WHEEL_RPM_MAX = 10.0       # Command cap: 20% of physical limit
...
# Controller Gain Parameters (Waypoint P-Controller)
Kp_lin = 1.3
Kp_ang = 4.0
max_lin = 0.25
max_ang = 1.5
wp_thresh = 0.10 # 10 cm threshold for reaching waypoints
...
class BBBHardware:
    def __init__(self):
        ...
        if GPIO_AVAILABLE:
            # Automatically configure pinmux for the GPIO pins
...
# ==========================================
# BNO055 IMU INTERFACE (9-DOF + Fused Yaw)
# ==========================================
class BBBIMU:
...
def main():
    ...
    try:
        ...
        csv_writer.writerow([
            ...
            "raw_enc_L", "raw_enc_R", 
            "ax", "ay", "az", "gx", "gy", "gz", "mx", "my", "mz", "fused_yaw_deg"
        ])
    except Exception as e:
    ...
    print(f"Starting Navigation Loop (dt={dt}s)...")
        ...
        while wp_idx < num_waypoints:
            ...
            if csv_file and csv_writer:
                csv_writer.writerow([
                    ...
                    round(mx, 2), round(my, 2), round(mz, 2), round(math.degrees(psi), 2)
                ])
                csv_file.flush()

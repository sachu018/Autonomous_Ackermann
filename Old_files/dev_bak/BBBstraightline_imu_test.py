...
class BBBHardware:
    ...
    def drive_motors(self, rpm_L, rpm_R):
        ...
        # Convert RPM to target DAC values scaled to physical maximum speed
...
def main():
    ...
    # Initialize CSV path trace logger
    import csv
    from datetime import datetime
    log_dir = "/home/debian/Robot/logs"
    os.makedirs(log_dir, exist_ok=True)
    timestamp_str = datetime.now().strftime("%Y%m%d_%H%M%S")
    csv_filename = os.path.join(log_dir, f"path_log_{timestamp_str}.csv")
    print(f"\n[Main] Path logging started. Saving trace to {csv_filename}")
    
    try:
        csv_file = open(csv_filename, mode='w', newline='')
        csv_writer = csv.writer(csv_file)
        csv_writer.writerow(["time_s", "x_m", "y_m", "yaw_deg", "cmd_rpm_l", "cmd_rpm_r", "enc_rpm_l", "enc_rpm_r"])
    except Exception as e:
        print(f"[Main] Warning: Could not create CSV log file: {e}")
        csv_file = None
        csv_writer = None
        
    start_time = time.time()
    
    # Initialize UDP socket for live web visualizer telemetry
    import socket
    import json
    ...
    try:
        while True:
            ...
            # Broadcast telemetry to UDP port 5005
            try:
                telemetry_data = {
                    "state": "CLOSED_LOOP",
                    ...
                    "cmd_rpm_l": round(rpm_L, 1),
                    "cmd_rpm_r": round(rpm_R, 1),
        ...
        if csv_file:
            try:
                csv_file.close()
                print(f"\n[Main] Path trace log saved successfully to {csv_filename}")
            except Exception:
                pass

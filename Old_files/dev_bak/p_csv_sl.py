...
# ==========================================
# target_distance removed  # Target drive distance in meters
...
def main():
    ...
    print(f"Starting Navigation Loop (dt={dt}s) | Target: {num_waypoints} waypoints")
    ...
    try:
        while wp_idx < num_waypoints:
            ...
            # --- Waypoint P-Controller Navigation Logic ---
            goal_x, goal_y = waypoints[wp_idx]
            dist_to_goal = math.hypot(goal_x - x, goal_y - y)
            
            # Waypoint proximity threshold (40 cm target bubble)
            wp_thresh = 0.1
            ...
            # Bound the yaw error to +/- 6 degrees for smooth P-controller steering
            yaw_err_bounded = max(math.radians(-6.0), min(math.radians(6.0), yaw_err))
            ...
            W = max(-max_ang, min(Kp_ang * yaw_err_bounded, max_ang))

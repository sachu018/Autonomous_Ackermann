...
p_straightline.py
-----------------
Drives the UGV in a straight line along the X-axis for a target distance (e.g. 4m).
Uses the P-controller speed/steering equations from the waypoint script, combined
with the verified hardware drivers, encoder interfaces, and logging loop of BBBstraightline_pid.py.
"""
...
# ==========================================
target_distance = 20.0  # Target drive distance in meters

a = 0.175             # Wheel radius (m)
d = 0.30              # Half wheelbase (m) - L/2
CPR = 2400            # Encoder counts per revolution (600 PPR * 4)
dt = 0.1              # Control loop rate (10 Hz matching BBBstraightline_pid)
...
# P-Controller Navigation Gains (From tuned waypoint follower)
Kp_lin = 1.3
Kp_ang = 1.8
max_lin = 0.25
max_ang = 1.5
...
class BBBHardware:
    def __init__(self):
        ...
        # Local speed PI loops (tuned to Kp=50.0, Ki=15.0)
    ...
    def drive_motors(self, rpm_L, rpm_R, enc_l=None, enc_r=None, dt=0.1):
        # Determine new direction relay states
        new_rev_L = rpm_L < 0
        new_rev_R = rpm_R < 0
        
        # Initialize relay tracking states if not present
        if not hasattr(self, 'current_rev_L'):
            self.current_rev_L = False
            self.current_rev_R = False
            
        # Check if direction is switching (Dry-Switching Relay Protection)
        if new_rev_L != self.current_rev_L or new_rev_R != self.current_rev_R:
            # 1. Instantly write 0V to BOTH motors to cut current
            self.write_dac(DAC_LEFT_ADDR, 0)
            self.write_dac(DAC_RIGHT_ADDR, 0)
            self.last_dac_L = 0
            self.last_dac_R = 0
            
            # 2. Wait 150ms for current to decay and motors to coast/stop
            time.sleep(0.15)
            
            # 3. Toggle relays while dry (no current flowing)
            self.set_gpios(new_rev_L, new_rev_R, brake_on=False)
            self.current_rev_L = new_rev_L
            self.current_rev_R = new_rev_R
            
            # 4. Wait another 50ms for relay contacts to settle
            time.sleep(0.05)
            
            # Reset speed PID loop integrals to prevent surge
            self.pid_speed_L.reset()
            self.pid_speed_R.reset()
        else:
            # Just ensure relays are in correct state
            self.set_gpios(new_rev_L, new_rev_R, brake_on=False)
...
# ==========================================
def main():
    # 1. Initialize Modular UGV Logger immediately to send UDP handshake to dashboard
    logger = UGVLogger()
    
    # 2. Initialize BNO055 and calibrate accelerometer zero-biases BEFORE energizing hardware
    imu = BBBIMU(None)
    
    # Initialize IMU Dead Reckoning (Inertial Navigation) variables
    x_imu, y_imu = 0.0, 0.0
    ...
    # Initialize Pose (Starting at origin facing 0 degrees)
    x, y = 0.0, 0.0
    psi = 0.0
    ...
    print(f"Starting Navigation Loop (dt={dt}s) | Target: {target_distance} meters")
    ...
    try:
        while True:
            ...
            # 5. --- Update Heading (Absolute IMU Fused Euler) ---
            ...
            # --- straightline P-Controller Navigation Logic ---
            dist_to_goal = target_distance - x
            yaw_err = wrap_to_pi(0.0 - psi)  # Target heading is 0.0 rad (straight forward)
            
            # If UGV reaches close to the target distance, break
            if dist_to_goal <= 0.05:
                break
                
            # Proportional velocity with cosine heading error scaling
            V = min(Kp_lin * dist_to_goal, max_lin)
            V = max(0.05, V)
            V *= max(0.2, math.cos(yaw_err))
            
            # Proportional steering with clamps
            ...
            # --- Drive Motors (Closed-Loop) ---
            hw.drive_motors(rpm_L, rpm_R, enc_l=enc_rpm_l, enc_r=enc_rpm_r, dt=dt_actual)
            
            print(f"Pose: x={x:>5.2f} y={y:>5.2f} yaw={math.degrees(psi):>+6.1f}° | Err: {dist_to_goal:.2f}m | Cmd: {rpm_L:+.1f},{rpm_R:+.1f}", end='\r')
        ...
        print("\n\n✅ Target distance reached. Straight line drive complete.")

...
ugv_p_waypoint.py
-----------------
Fresh, optimized proportional (P) waypoint tracking controller for BeagleBone Black.
Loads waypoints from a CSV file, tracks them using relative IMU heading, and
broadcasts live telemetry via the modular UGVLogger.
"""
...
# Controller Parameters (Tuned gains)
Kp_lin = 0.8          # Linear velocity proportional gain
Kp_ang = 1.3          # Angular steering proportional gain
max_lin = 0.25        # Max linear velocity (m/s)
max_ang = 1.5         # Max angular velocity (rad/s)
wp_thresh = 0.4       # Waypoint reach threshold (meters)
...
# ==========================================
class PController:
    """Pure Proportional (P) Controller for outer loop path tracking"""
    def __init__(self, Kp, max_output, min_output):
        self.Kp = Kp
...
class PIController:
    """Proportional-Integral (PI) Controller for inner wheel speed loops"""
    def __init__(self, Kp, Ki, max_output, min_output, max_integral=150.0):
        self.Kp = Kp
        self.Ki = Ki
        self.max_output = max_output
        self.min_output = min_output
...
class BBBHardware:
    def __init__(self):
        ...
        self.pid_speed_L = PIController(Kp=50.0, Ki=15.0, max_output=4095-1100, min_output=-250)
        self.pid_speed_R = PIController(Kp=50.0, Ki=15.0, max_output=4095-1100, min_output=-250)
...
class BBBIMU:
    def __init__(self, bus):
        ...
        try:
            ...
            self.sensor.mode = 0x0C  # 6-DOF IMU mode (magnetic immune)
            self.initialized = True
            print("[IMU] BNO055 IMU initialized in 9-DOF NDOF mode.")
...
def main():
    ...
    print(f"Starting Waypoint Navigation Loop (dt={dt}s) | Total Waypoints: {num_waypoints}")
    ...
    try:
        while wp_idx < num_waypoints:
            ...
            # 4. Log telemetry (1 Hz temperature query to protect I2C bandwidth)
            ...
            # 5. Waypoint Tracking P-Controller Logic
            ...
            # Bound yaw error locally to +/- 6 degrees for smooth steering

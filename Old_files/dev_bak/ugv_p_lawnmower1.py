...
Kp_ang = 1.1          # Angular steering proportional gain
...
def main():
    logger = UGVLogger()
    csv_path = "/home/debian/Robot/lawnmower_waypoints1.csv"
    ...
    try:
        while wp_idx < num_waypoints:
            ...
            # Forward speed controller (using raw yaw error)
            V = min(Kp_lin * dist_to_goal, max_lin)
            V = max(0.05, V)
            V *= max(0.2, math.cos(yaw_err))
            
            # Steering controller (using raw yaw error, capped globally by max_ang)

...
**Mitigation applied (RPi-side config only, user's chosen option):** doubled `CRUISE_SPEED_MPS` 0.12→0.24 and `MIN_SPEED_MPS` 0.06→0.12 in `rover_config.py`. **Not yet retested on hardware.**
...
**Tooling built:** `RPi_companion/calibrate_steering_pots.py` — walks full-left-lock / full-right-lock / center, printing live `angle_L`/`angle_R`/`delta` (non-blocking Enter-to-capture) so the operator can watch the two numbers agree before capturing the center point with a string-line across both wheels simultaneously. Doesn't need raw ADC in the UART frame (not present in `rpi_link.c`'s format) — instead exactly inverts `_MapToAngle()` using the current firmware constants to recover the true raw ADC at each captured position (round-trip verified < 1e-6 error over 1000 samples). Prints the 6 new `ADC_L/R_MIN/CENTER/MAX_RAW` values to paste into `ackermann_config.h`. Deployed to the RPi. **Two field runs done** (§3.16 below has the follow-up findings from those runs); new calibration values have not been committed to `ackermann_config.h` yet — the user chose to fix the actuator control first (§3.16) before locking in a recalibration, since the actuator's own approach-direction hysteresis was making "center" hard to measure repeatably in the first place.

### 3.16 Actuator Hysteresis Found, Then a Full Bang-Bang/PID Redesign

**Hysteresis discovered.** Two calibration runs of `calibrate_steering_pots.py` disagreed on where "center" was by **~2.9°** — close to the size of the thing being measured. Investigating why: settling to commanded-zero from full-left-lock vs. from full-right-lock left the wheels in measurably different physical positions — **+2.26° vs. -2.27° under the old calibration (a 4.53° gap)**, and re-mapping the same raw readings through the *new* calibration barely changed it (4.29°) — proving this is an independent actuator-control effect, not a sensor-calibration one. Root cause: `main.c`'s actuator control was pure bang-bang — 100% speed with no deceleration until the deadband check trips — so however much it overshoots past that trip point depends on approach direction and momentum, not a fixed physical point.

**Redesign, agreed over several rounds of discussion:**
- **Centering uses both pots individually; the differential keeps using the average.** `Ackermann_ComputeRPM()` is unchanged (still `steer.delta`, the user's explicit call — that's the standard bicycle-model convention and it's fine there). Only the actuator's own "am I centered" decision changes: when `|target_steer_deg| < CENTER_TARGET_EPS` (1.0°, i.e. returning to straight — the condition that actually drives the field drift), it now requires **both** `angle_L` and `angle_R` individually within `STEER_DEADBAND_DEG`, not just their average — directly closes the gap the user identified (two very different per-wheel readings can average to the same `delta`, hiding a real disagreement). Off-center targets keep the original delta-only check, since `angle_L != angle_R` is *expected* there (real Ackermann inner/outer divergence during a turn).
- **`steer_pid.c` activated** (dead code since the Phase 2 port — `SteerPID_Init()` was called at boot but `SteerPID_Update()` was never called). New two-zone control: bang-bang (100%) while `|error| > STEER_PROP_ZONE_DEG` (4°), PID takeover inside that zone — directly targets the overshoot that caused the hysteresis. `KP/KI/KD_STEER` (5.0/0.1/0.5, BBB-ported) are exercised on this hardware for the first time and flagged as unlikely to be well-tuned as-is.
- **`ACT_MIN_DUTY_PCT = 40.0f`** floors the PID's output (same role as `THR_FWD_MIN_DAC` for the rear wheels — a small computed duty can be too weak to break the actuator's static friction and stall short). Identified the actual hardware first via web search: almost certainly the **PA-12-300-1500** (300mm/7mm-s/12V — matches multiple Indian storefronts: Robu, Probots, Rees52, RoboticsDNA). Datasheets gave rated current (~3.8A loaded), full-load speed drop (7→5.5mm/s), a **10% duty-cycle rating**, and a self-locking screw — but no minimum-moving-duty spec (expected; same situation as the rear-motor stiction floor). 40% is a starting guess, not a measured value.
- **Watchdog against unsatisfiable centering.** The per-wheel check only has a solution if the real `angle_L`/`angle_R` mismatch is under `2 * STEER_DEADBAND_DEG` (3° now). If that mismatch ever grows (pot drift, a failing sensor), the PID could hunt indefinitely — and given the actuator's 10% duty-cycle rating just found, that's a real damage risk, not just an inconvenience. `STEER_CENTER_WATCHDOG_US` (2.5s) bounds the wait before falling back to the always-satisfiable delta-only check.
- **`STEER_DEADBAND_DEG` narrowed 2.5° → 1.5°** — the old value was sized to tolerate bang-bang's overshoot; the PID's gentler approach makes a tighter tolerance worth trying. Flagged to widen back up if the actuator ever hunts/oscillates right at the new boundary.
